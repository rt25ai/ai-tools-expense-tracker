          ...
          <div key={month.key} className="space-y-2">
            <div className="flex items-center justify-between gap-4 text-sm">
              ...
              <div className="flex items-center gap-3 text-zinc-400">
                <span>{formatCurrencyIls(month.total)}</span>
                <span className="text-zinc-600">מתוך</span>
                <span>{formatCurrencyIls(month.budget)}</span>
              </div>
            </div>
            <div className="relative h-3 rounded-full bg-white/[0.05]">
              ...
              <div className="absolute inset-y-0 right-0 rounded-full bg-cyan-400" style={{ width: totalWidth }} />
            </div>
          </div>
        );
      })}
