...
import type { ReportMonth, ReportYear } from "@/lib/dashboard-data";
import { formatCurrencyIls } from "@/lib/formatters";
import { applyMonthlyBudgetToMonths, applyMonthlyBudgetToYearReport, DEFAULT_MONTHLY_BUDGET } from "@/lib/monthly-budget";
...
export function ReportsPageClient({
  ...
  months: ReportMonth[];
  yearReports: ReportYear[];
}) {
  const monthlyBudget = useMonthlyBudget(DEFAULT_MONTHLY_BUDGET);
  const budgetedMonths = useMemo(
    () => applyMonthlyBudgetToMonths(months, monthlyBudget),
    [months, monthlyBudget],
  );
  const budgetedYearReports = useMemo(
    () => yearReports.map((report) => applyMonthlyBudgetToYearReport(report, monthlyBudget)),
    [yearReports, monthlyBudget],
  );
  const latestMonths = useMemo(
    () => [...budgetedMonths].sort((left, right) => right.key.localeCompare(left.key)).slice(0, 6),
    [budgetedMonths],
  );
  ...
  return (
    <>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        {budgetedYearReports.map((report) => (
          <Link
            key={report.year}
            href={yearReportHref(report.year)}
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2 className="mt-2 text-3xl font-semibold text-white">{report.year}</h2>
              </div>
