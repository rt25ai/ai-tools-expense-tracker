  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-1 flex-col gap-3 sm:flex-row">
          <div className="relative flex-1">
            <Search className="pointer-events-none absolute end-3 top-1/2 size-4 -translate-y-1/2 text-zinc-500" />
            <Input
              value={globalFilter}
              onChange={(event) => setGlobalFilter(event.target.value)}
              placeholder="חפש ספק, תיאור, או קטגוריה"
              className="h-11 border-white/10 bg-white/[0.04] pe-9 text-zinc-100 placeholder:text-zinc-500"
            />
          </div>
          <Select value={sourceFilter} onValueChange={setSourceFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="email-imported">מיוצא מאימייל</SelectItem>
              <SelectItem value="ai-extracted">מחולץ על ידי AI</SelectItem>
            </SelectContent>
          </Select>
            ...
            {table.getRowModel().rows.length ? (
              ...
              <TableRow className="border-white/6">
                <TableCell colSpan={columns.length} className="h-24 text-center text-zinc-500">
                  אין עסקאות התואמות לפילטרים הנוכחיים.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <div className="flex flex-col gap-3 rounded-[24px] border border-white/8 bg-white/[0.02] px-4 py-3 text-sm text-zinc-400 sm:flex-row sm:items-center sm:justify-between">
        <p>
          מציג {table.getRowModel().rows.length} מתוך {filteredData.length} שורות מסוננות
        </p>
